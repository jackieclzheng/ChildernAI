    ...
    def play_voice_prompt(self, text):
        ...
        if text in self.voice_cache:
            try:
                ...
                try:
                    ...
                    print(f"QSound播放失败，尝试使用pygame: {str(e)}")

                    # 如果QSound失败，尝试使用pygame
                    try:
        ...
        try:
            ...
            try:
                ...
                print(f"pygame播放失败，尝试使用QSound: {str(e)}")

                # 如果pygame失败，尝试使用QSound
                try:
                    sound = QSound(temp_file_path)
                    sound.play()
                    ...
                    QTimer.singleShot(10000, delete_temp_file)
                    print(f"使用QSound播放edge-tts语音成功: {text}")
                    return True
                except Exception as e:
                    print(f"QSound播放也失败: {str(e)}")
                    # 继续尝试其他方法
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        QTimer.singleShot(10000, delete_temp_file)
                        print(f"使用QSound播放gTTS语音成功: {text}")
                        return True
                    except Exception as e:
                        print(f"QSound播放也失败: {str(e)}")
                        # 继续尝试其他方法
